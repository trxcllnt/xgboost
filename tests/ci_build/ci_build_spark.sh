...
then
    ...
    echo "Canceled docker push ${DOCKER_CACHE_REPO}/${DOCKER_IMG_NAME}:${BRANCH_NAME}"
...
echo "${DOCKER_BINARY} run --rm --pid=host -v ${WORKSPACE}:/workspace -w /workspace ${USER_IDS} ${CI_DOCKER_EXTRA_PARAMS[@]} ${DOCKER_IMG_NAME} ${COMMAND[@]}"
