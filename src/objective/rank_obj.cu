...
struct LambdaRankParam : dmlc::Parameter<LambdaRankParam> {
...
class PairwiseLambdaWeightComputer {
 public:
  ...
  static const char *Name() { return "rank:pairwise"; }
...
#endif
{
 public:
  ...
  static const char *Name() { return "rank:ndcg"; }
...
#endif
{
 ...
 public:
  ...
  static const char *Name() { return "rank:map"; }
...
class SortedLabelList : SegmentSorter<float> {
 ...
 public:
  ...
  void Sort(const bst_float *dlabels, uint32_t labels_size, const std::vector<uint32_t> &groups) {
    this->SortItems(dlabels, labels_size, groups);
  }
  ...
  void ComputeGradients(const bst_float *dpreds,    // Unsorted predictions
                        ...
                        const bst_float *dweights,  // Group weights
                        int iter,
    ...
    dh::LaunchN(device_id, niter, nullptr, [=] __device__(uint32_t idx) {
      ...
      float weight = dweights ? dweights[group_idx - 1] : 1.0f;
...
class LambdaRankObj : public ObjFunction {
 public:
  ...
  void GetGradient(const HostDeviceVector<bst_float>& preds,
    ...
    if (devices.Size()) {
      ComputeGradientsOnGPU(preds, info, iter, out_gpair, gptr, devices);
    } else {
      // Revert back to CPU
#endif
 ...
 private:
  ...
  void ComputeGradientsOnGPU(const HostDeviceVector<bst_float>& preds,
    ...
    // Shard all the device vectors to copy them to the device
    auto dist = GPUDistribution(devices);
    if (out_gpair->Devices().IsEmpty()) {
      out_gpair->Shard(dist);
    }
    if (preds.Devices().IsEmpty()) {
      preds.Shard(dist);
    }
    if (info.labels_.Devices().IsEmpty()) {
      info.labels_.Shard(dist);
    }
    if (info.weights_.Size() && info.weights_.Devices().IsEmpty()) {
      info.weights_.Shard(dist);
    }

    out_gpair->Resize(preds.Size());

    auto d_preds = preds.ConstDevicePointer(device);
    auto d_gpair = out_gpair->DevicePointer(device);
    auto d_labels = info.labels_.ConstDevicePointer(device);
    auto d_weights = info.weights_.Size() ? info.weights_.ConstDevicePointer(device) : nullptr;

    SortedLabelList slist(param_);

    // Sort the labels within the groups on the device
    slist.Sort(d_labels, info.labels_.Size(), gptr);
    ...
    slist.ComputeGradients<LambdaWeightComputerT>
      (d_preds, d_labels, d_weights, iter, d_gpair, weight_normalization_factor);
  }
#endif
