...
class SortedLabelList {
 ...
 public:
  ...
  void Sort(const HostDeviceVector<bst_float> &dlabels) {
    dsorted_labels_.assign(dlabels.tcbegin(device_id_), dlabels.tcend(device_id_));
    ...
    thrust::gather(doriginal_pos_.begin(), doriginal_pos_.end(),
                   dlabels.tcbegin(device_id_), dsorted_labels_.begin());
  }
  ...
  void ComputeGradients(const bst_float *dpreds,
    ...
    auto dweights = num_weights ? weights.ConstDevicePointer(device_id_) : nullptr;
...
class LambdaRankObj : public ObjFunction {
 public:
  ...
  void GetGradient(const HostDeviceVector<bst_float>& preds,
    ...
    if (devices.Size() && LambdaWeightComputerT::SupportOnGPU()) {
      LOG(DEBUG) << "Computing pairwise gradients on GPU.";
      CHECK(devices.Size() == 1);
      auto dev_id = *devices.begin();
      dh::safe_cuda(cudaSetDevice(dev_id));
      ...
      auto d_preds = preds.ConstDevicePointer(dev_id);
      auto d_gpair = out_gpair->DevicePointer(dev_id);

      if (!slist_) {
        slist_.reset(new SortedLabelList(dev_id, param_));
      }
