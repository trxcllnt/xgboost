...
abstract class FilePartitionReaderFactory extends PartitionReaderFactory {
  ...
  def buildColumnarReader(partitionedFile: PartitionedFile): PartitionReader[GpuColumnBatch] = {
    throw new UnsupportedOperationException("Cannot create columnar reader.")
  }
}

private[this] case class PartitionedFileReader[T](file: PartitionedFile,
